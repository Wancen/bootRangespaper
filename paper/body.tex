...
\wancen{or identify putative transcription factor binding site according to enriched peak regions,}
...
%The workflow of \bootranges includes downstream analysis with \plyranges.
...
\wancen{where we bootstrapped the SNPs}.
...
(\cref{fig:result}A-B). \wancen{Here we use $z$ score to measure the distance between the expected
value and the observed one according to the standard deviations. 
Overlap rate was defined as the proportion of
SNPs has peaks nearby within 10kb.} 
...
where $L_b$ is too close to $L_s$ \wancen{results failed to random select the blocks place}.
...
a scaled version of the change in the width of the \wancen{neighbouring}
bootstrap distribution over $L_b$,
and \wancen{examing conserved spatial pattern by the inter-feature distance distribution} (Supplementary Methods).
...
(\cref{fig:result}C-D).
\wancen{Plots recommened $-\log_{10}(p) = 8$ and $\textrm{|logFC|} = 2$
as optimized thresholds where the $z$ score was highest (\cref{fig:suppfig}D-E).}
...
\wancen{Additionally, the bootstrapped gene-peaks
correlation per gene can be computed to idenitfy candidate gene-promoter pairs(\cref{fig:suppfig}G-H).}
...
ENCODE kidney and bladder ChIP-seq \wancen{from GSC Github}. The average time to
