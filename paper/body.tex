...
there is an significant association between two sets of regions, 
...
genomic regions in the genome, possibly considering a set of
excluded regions.
...
random sets by sampling large blocks of regions from the
original set with replacement, as proposed for 
genomic regions by \citet{bickel2010subsampling} in Genome Structure
Correlation (GSC).
...
\caption{Overview of \bootranges workflow.
  ...
  B) Testing overlaps of regions in \bm{$x$} with regions in
...
and the inter-region distances distribution to assess conserved
...
Shuffling of genomic regions (Supplementary \cref{sec:shuffle})
...
\bootranges efficiently generates null models of genomic regions preserving 
