...
excluded regions \rev{where ranges should not be placed}.
...
\caption{Schematic overview.
  A) Bootstrapping blocks of length $L_b$ \wancen{from randomly selected position and move to a tile block across chromosome within same states. Color represents segmentation states.} 
  B) \wancen{Workflow of testing null hypothesis of true biological question between \bm{$x$} and \bm{$y$} using \bootranges. First, deriving \bootranges of \bm{$y$}. Then, calculating the distribution of interested statistics between \bm{$x$} and \bootranges. Finally, permutation test can be performed to access the null hypothesis.}} 
\label{fig:framework}
%\vspace{-0.5cm}
\end{figure}
...
\wancen{As seen in applications of \citet{bickel2010subsampling}, the effect
of segmentation did not greatly alter conclusions. While observed overlap was significant across many combinations of
various segmentation methods and $L_b$ according to empirical p-value, the variance of the
bootstrap statistics distribution and the resulting $z$ score varied greatly across segmentation choice, though depending on block length (\cref{fig:result}A-B). It may vary more in other contexts. }
...
\wancen{To assess the accuracy of estimating statistics, we compared \bootranges result with shuffling in simulation to complement previous real data findings. Details are in \cref{sec:sim}. We showed that \bootranges can achieve low false positive rate (FPR) as desired. While shuffling generate distribution of statistics with similar mean as original distribution, but smaller variance. Therefore the FPR are relatively high and will overestimate how unlikely the observed statistics is (\cref{fig:simulation}).}
