...
genomic features in the genome, possibly considering a set of
excluded regions.
...
random sets by sampling large blocks of features from the
original set with replacement, as proposed for 
genomic features by \citet{bickel2010subsampling} in Genome Structure
Correlation (GSC).
...
The input for the workflow is features \bm{$x$} and
...
and the inter-feature distances distribution to assess conserved
