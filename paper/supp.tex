...
a Negative Binomial likelihood, \wancen{and matchRanges utilizing covariate matching method.}
...
density \mike{how many states} \wancen{usually in three states}. We then used the
...
Mikhail's vignette for which one to cite} \wancen{No matter which one, only one paper was cited from Mikhail's vignette, \citep{amemiya2019encode}},
...
\wancen{I derived this conclusion to make genome-wise and block-wise z score comparative. Because the $\bar{s}$ is similar to both analysis, then only changes come from SD. In simulation practice, I found out they ususally scaled by $\sqrt{L_b}$. But in theory, as $\text{SD}_R$ is for genome-wise standard deviation, then $\text{SD}_{R_b} = \sqrt{\frac{1}{R_b-1} \sum_{r=1}^{R_b} [s_r - \bar{s}]^2}
\approx \sqrt{\frac{L_b}{L_G}} \times \sqrt{\frac{1}{R}\sum_{r=1}^{R_b} [s_r - \bar{s}]^2}$. Actually, I am not sure should we mention that in text.}
...
\While{$r \leq R$}{
  ...
  \texttt{bootstrap[r]} $\gets$ Shift regions in $\bm{y}$ as in Algorithm \ref{alg:unseg}
}
Concatenate \texttt{bootstrap} regions across iterations $r$\\
\end{algorithm}
...
generated \mike{bootstrap?}\wancen{Yes, do we need to mention bootstrapped between "generated null"?} null sets should have similar spatial properties with
...
inter-region distances between the original and \mike{bootstrap?}
...
null model inter-region distance with bin size = 0.3 \mike{why this number?} 
\wancen{we have to specify a number otherwise histogram will automatically choose different bin size for null distributions generated from different $L_b$ and $L_s$ settings. 0.3 was chosen by visualization that looks good for all settings}.
...
inter-region distances, $L_b \in [300kb,600kb]$ was
...
\wancen{For each generated null set, we found all the predicted values into the range (-8,10) with 1 as the increment. Then conditional density plots can be plotted across all null sets. Here, $z$ measured the distance between predicted overlap counts using observed sets and the null sets.}
...
\wancen{The following code shows how the analysis of Fig 2C can be
performed as the downstream task of \bootranges. Suppose $x$ is DE genes and $bootRanges$ is bootstrapped DA peaks. For each gene within each iteration, we extract total number of peaks that overlapped with it. Then, fitting splines, predicting 2000 overlap counts on original logFC range, and finding the predictions belongs to the specified range.}
...
TSS). \mike{distance was calculated from TSS right?} \wancen{No, I think maxgap in plyranges refer to intervel distance. It may be calculated by using gene's last pair position.}
...
\caption{$L_b$ selection assessment. A) A pseudo-metric $d^*$
  ...
  log2(inter-region distance+1) density. The more \wancen{bootstrapped}
  null sets' density
...
\caption{
  ...
  A) \textit{z} score over -log10 (\textit{p}-value) \wancen{of SNPs} \mike{of
    what? need to explain again what you are doing here, ... when
    thresholding ... by p-value and computing overlap} in the liver
...
\caption{Correlation analysis of Chromium Single Cell
  ...
  type. \wancen{The correlation of this gene-promoter pair was 
  0.992 and was the highest among all the pairs.} \mike{was this the highest?}}
\label{fig:suppfig3}
\end{figure*}
