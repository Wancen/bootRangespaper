...
\rev{We first introduce a few choices regarding bootstrapping
  ...
  entire genome.
  \citet{bickel2010subsampling}
  demonstrated the advantages of using segmentation via simulation.
  If the purpose is block bootstrapping ranges within a smaller set of
  sequences, such as transcripts, then the unsegmented algorithm would
  be sufficient. 
  Finally, for the unsegmented bootstrap, the block bootstrap will be
  preferred over block permutation, as the latter provides additional
...
\rev{
  A small window of chromosome 4 was selected as one ranges set
  $\bm{x}$. Then we make 50 ranges sets $\bm{y}s$ whose features
  located in clumps. The number of gene within each cluster $\sim
  poisson(5)$ and the start position is uniformly chosen within a
  relatively larger window. Therefore, we would expect there is no
  association between $\bm{x}$ and $\bm{y}$. In order to compare the
  accuracy of shuffling and block bootstrapping, we shuffled and block
  bootstrapped 50 times within a relatively larger window. One
  realization is shown in (\cref{fig:simulation} A). Two situations of
  interested statistics are considered. One is simply based on genomic
  location - total number of overlaps, another one is based on the
  metadata column of one ranges set - total score. We computed those
  statistics between $\bm{x}$ and simulated, shuffling, bootstrapping
  dataset across 50 iterations (\cref{fig:simulation} B, C). In both
  case, the distribution of statistics using simulated dataset are
  highly overlapped with block bootstrapping. While shuffling generate
  distribution of statistics with similar mean, but smaller
  variance. To access the accuracy, we take the right 5\% tail of
  shuffling and found out 26\% simulation distribution falls in it,
  which represent a high false positive rate(FPR). When we take the
  tight 5\% of bootstrapping, FPR is around 6\% that as we
  desired. Therefore, we conclude that shuffling will overestimate how
  unlikely the observed statistics is and lead to misinterpretation of
  enrichment.
}
...
\caption{\rev{Comparison of shuffling and block bootstrapping in
