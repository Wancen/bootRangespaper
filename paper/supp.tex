...
a Negative Binomial likelihood, \wancen{and matchRanges \citep{Davis2022matchranges} utilizing covariate matching method.}
...
\wancen{CBS, implemented in the \textit{DNAcopy} package, was used to recursively split
...
ENCODE excluded regions \wancen{\citep{amemiya2019encode}} \mike{cite Kundaje paper for this, see
Mikhail's vignette for which one to cite} \wancen{No matter which one, only this paper was cited from Mikhail's vignette},
...
generated null models, \wancen{same as bootstrap distribution}, should have similar spatial properties with
...
the GEO accession: GSE164870. \wancen{All the peaks here were filtered by FDR $\leq 0.05$.}
...
used to define this set} \wancen{I check the largest FDR is 0.049}
...
original and
null model inter-region distance with bin size = 0.3 under various $L_b$ and $L_s$ settings. \mike{why this number?} 
\wancen{we have to specify a number otherwise histogram will automatically choose different bin size for null sets generated from different $L_b$ and $L_s$ settings. 0.3 was chosen by visualization that looks good for all settings}. Under this aspect, $L_b \in [200kb,600kb]$ was
...
%  bootstrap distribution of the overlap count ($R = 1,000$).
  A) \wancen{\textit{z} score against SNPs -log10 (\textit{p}-value)  in the liver
...
null sets($R = 1,000$) in terms of SD, was derived.} \mike{of
    what? need to explain again what you are doing here, ... when
    thresholding ... by p-value and computing overlap} B) \textit{z} score against gene expression logFC of the
