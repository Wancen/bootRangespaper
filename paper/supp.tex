...
\KwData{Regions ($\bm{y}$), Block length ($L_b$), Length of total genome ($L_C$),
  ...
  and $\alpha_i$ representing ranges belong to state $i$}
\KwResult{$\texttt{bootRanges}$ object}
$r \gets 1$\\
\While{$r \leq R$}{
  \ForEach{segmentation state $i$}{
    $L_s^i \gets \sum_{j\in \alpha_i} L_j$ \\
    $L_b^i \gets L_b \cdot L_s^i / L_C$ \\
    $n_b^i \gets \sum_{j\in \alpha_i} \text{ceiling}(L_{j} / L_b^i)$ \\
    \texttt{rearranged blocks} $\gets$ Generate $n_b^i$ tiling blocks start site \\
