...
sets of regions $\bm{y_r}$ within $R_b = R \times \sum_{c=1}^{C} \textrm{ceiling}(L_c / L_b)$ blocks, for the chromosome $c$ length $L_c$.
...
the original sets, e.g. inter-region distances. Here we use
...
original and
null model inter-region distance with bin size = 0.3 under various
...
\caption{$L_b$ selection assessment. A) A pseudo-metric $d^*$
  ...
  a particular null set's density
  plot overlapped with observed regions, the better we conserved the
