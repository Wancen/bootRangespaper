...
respectively.  Additionally, we could use $z$ score to measure the
distance between the observed statistics and the bootstrap statistics
distribution in terms of standard deviations (SD).  Specifically, the
...
\begin{algorithm}[H]
  ...
  \While{$r \leq R$}{
    \textbf{rearranged blocks:} Generate consecutive tiling blocks with width = $L_b$\\
    \uIf{permutation}{
\textbf{random blocks:} Sample blocks without replacement from rearranged blocks
}\ElseIf{bootstrap}{ 
         $n_b = \sum_{c=1}^{C} \text{ceiling}(L_c / L_b)$\\
         \textbf{random blocks:} Generate $n_b$ blocks with replacement and number of blocks per chromosome is proportional to $L_c$\\ 
      }
