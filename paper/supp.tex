...
\KwData{Regions ($\bm{y}$), Block length ($L_b$), Length of total genome ($L_G$),
  Bootstrap iterations ($R$), Segmentation states $\mathcal S$,
  Segmentation regions $\mathcal{R}_s$ (the set of all regions
  belonging to state $s$), and with $L_r^s$ representing the width of region $r$
  in state $s$}
\KwResult{$\texttt{bootRanges}$ object}
$r \gets 1$\\
\While{$r \leq R$}{
  \ForEach{segmentation state $s \in \mathcal{S}$}{
    $L_s \gets \sum_{r \in \mathcal{R}_s} L_r^s$ \\
    $L_b^s \gets L_b \cdot L_s / L_G$ \\
    $n_b^s \gets \sum_{r \in \mathcal{R}_s} \text{ceiling}(L_r^s / L_b^s)$ \\
    \texttt{rearranged blocks} $\gets$ Generate $n_b^s$ consecutive tiling blocks along $\mathcal{R}_s$ \\
    \texttt{random blocks} $\gets$ Generate $n_b^i$ blocks start site with replacement and and number of blocks per ranges is proportional to $L_j$ \\
...
\begin{algorithm}
\SetNoFillComment
\caption{Segmented block bootstrap with fixed block length}\label{alg:supp_fixedlb}
\KwData{Regions ($\bm{y}$), Block length ($L_b$), Length of total genome ($L_C$),
  Bootstrap iterations ($R$), Segmentation regions with $L_j$ representing each ranges width
  and $\beta$ representing total number of ranges}
\KwResult{$\texttt{bootRanges}$}
$r \gets 1$\\
\While{$r \leq R$}{
  $n_{j} \gets \text{ceiling}(L_{j} / L_b)$ \\
  $n_b \gets \sum_{j=1}^{\beta} n_j$ \\
  \texttt{rearranged blocks} $\gets$ Generate $n_b$ tiling blocks start site by order with width = $L_b$  \\
  \texttt{random blocks} $\gets$ Generate $n_b$ blocks start site with replacement and number of blocks per ranges is proportional to $L_j$ \\
  \ForEach{segmentation state $i$}{
